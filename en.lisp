;;; English translations (base language)
;;; This is the authoritative source for all translation keys

(
 ;; Language metadata (used for language picker)
 (:lang-name . "English")
 (:lang-flag . "🇬🇧")

 ;; Navigation
 (:nav-translate . "Translate")
 (:nav-review . "Review")
 (:nav-dashboard . "Dashboard")
 (:nav-logout . "Logout")

 ;; App title and branding
 (:app-title . "Hiisi")
 (:app-tagline . "Learn languages with AI-powered translations")

 ;; Landing page
 (:landing-hero-title . "Master Any Language")
 (:landing-hero-subtitle . "AI-powered translations with detailed grammar explanations and spaced repetition flashcards")
 (:landing-cta . "Get Started")
 (:landing-feature-1-title . "Smart Translations")
 (:landing-feature-1-desc . "Get detailed grammatical explanations alongside translations")
 (:landing-feature-2-title . "Spaced Repetition")
 (:landing-feature-2-desc . "Remember what you learn with our SM-2 flashcard system")
 (:landing-feature-3-title . "Track Progress")
 (:landing-feature-3-desc . "Monitor your learning journey with detailed statistics")

 ;; Auth pages
 (:auth-register . "Register")
 (:auth-login . "Login")
 (:auth-logout . "Logout")
 (:auth-email . "Email")
 (:auth-password . "Password")
 (:auth-password-again . "Confirm Password")
 (:auth-display-name . "Display Name")
 (:auth-register-title . "Register to Hiisi")
 (:auth-register-subtitle . "Create a new account")
 (:auth-login-title . "Login to Hiisi")
 (:auth-login-subtitle . "Enter your credentials")
 (:auth-forgot-password . "Forgot password?")
 (:auth-no-account . "Don't have an account?")
 (:auth-have-account . "Already have an account?")

 ;; Translation page
 (:translate-title . "Translate")
 (:translate-input-label . "Enter text to translate")
 (:translate-input-placeholder . "Type something...")
 (:translate-from . "From")
 (:translate-to . "To")
 (:translate-submit . "Translate")
 (:translate-save . "Save to Flashcards")
 (:translate-saved . "Saved!")
 (:translate-like . "Like")
 (:translate-dislike . "Dislike")
 (:translate-char-limit . "characters remaining")

 ;; Flashcard review
 (:review-title . "Review")
 (:review-start . "Start Review")
 (:review-no-cards . "No cards due for review")
 (:review-show-answer . "Show Answer")
 (:review-again . "Again")
 (:review-hard . "Hard")
 (:review-good . "Good")
 (:review-easy . "Easy")
 (:review-progress . "Card ~a of ~a")
 (:review-complete . "Review Complete!")
 (:review-complete-msg . "You reviewed ~a cards")
 (:review-streak . "day streak")

 ;; Dashboard
 (:dashboard-title . "Dashboard")
 (:dashboard-cards-due . "Cards Due Today")
 (:dashboard-total-cards . "Total Cards")
 (:dashboard-subscription . "Subscription")
 (:dashboard-subscribed-until . "Subscribed until")
 (:dashboard-not-subscribed . "Not subscribed")
 (:dashboard-recent . "Recent Translations")
 (:dashboard-no-translations . "No translations yet")
 (:dashboard-streak . "Current Streak")
 (:dashboard-days . "days")

 ;; Subscription
 (:subscribe-title . "Choose Your Plan")
 (:subscribe-monthly . "Monthly")
 (:subscribe-annual . "Annual")
 (:subscribe-monthly-price . "€10/month")
 (:subscribe-annual-price . "€100/year")
 (:subscribe-monthly-desc . "600 translations per month")
 (:subscribe-annual-desc . "Save €20 - two months free!")
 (:subscribe-cta . "Subscribe")
 (:subscribe-success . "Subscription Successful!")
 (:subscribe-manage . "Manage Subscription")

 ;; Errors and status
 (:error-generic . "Something went wrong")
 (:error-not-found . "Page not found")
 (:error-unauthorized . "Please log in to continue")
 (:error-subscription-required . "Subscription required")
 (:loading . "Loading...")
 (:empty-state . "Nothing here yet")

 ;; Footer
 (:footer-privacy . "Privacy Policy")
 (:footer-terms . "Terms of Service")
 (:footer-contact . "Contact")
)
