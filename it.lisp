;;; Italian translations

(
 ;; Language metadata (used for language picker)
 (:lang-name . "Italiano")
 (:lang-flag . "🇮🇹")

 ;; Navigation
 (:nav-translate . "Traduci")
 (:nav-review . "Ripassa")
 (:nav-dashboard . "Dashboard")
 (:nav-logout . "Esci")

 ;; App title and branding
 (:app-title . "Hiisi")
 (:app-tagline . "Impara le lingue con traduzioni basate su IA")

 ;; Landing page
 (:landing-hero-title . "Padroneggia qualsiasi lingua")
 (:landing-hero-subtitle . "Traduzioni con IA con spiegazioni grammaticali dettagliate e flashcard a ripetizione spaziata")
 (:landing-cta . "Inizia")
 (:landing-feature-1-title . "Traduzioni intelligenti")
 (:landing-feature-1-desc . "Ottieni spiegazioni grammaticali dettagliate insieme alle traduzioni")
 (:landing-feature-2-title . "Ripetizione spaziata")
 (:landing-feature-2-desc . "Ricorda ciò che impari con il nostro sistema di flashcard SM-2")
 (:landing-feature-3-title . "Monitora i progressi")
 (:landing-feature-3-desc . "Segui il tuo percorso di apprendimento con statistiche dettagliate")

 ;; Auth pages
 (:auth-register . "Registrati")
 (:auth-login . "Accedi")
 (:auth-logout . "Esci")
 (:auth-email . "Email")
 (:auth-password . "Password")
 (:auth-password-again . "Conferma password")
 (:auth-display-name . "Nome visualizzato")
 (:auth-register-title . "Registrati su Hiisi")
 (:auth-register-subtitle . "Crea un nuovo account")
 (:auth-login-title . "Accedi a Hiisi")
 (:auth-login-subtitle . "Inserisci le tue credenziali")
 (:auth-forgot-password . "Password dimenticata?")
 (:auth-no-account . "Non hai un account?")
 (:auth-have-account . "Hai già un account?")

 ;; Translation page
 (:translate-title . "Traduci")
 (:translate-input-label . "Inserisci il testo da tradurre")
 (:translate-input-placeholder . "Scrivi qualcosa...")
 (:translate-from . "Da")
 (:translate-to . "A")
 (:translate-submit . "Traduci")
 (:translate-save . "Salva nelle flashcard")
 (:translate-saved . "Salvato!")
 (:translate-like . "Mi piace")
 (:translate-dislike . "Non mi piace")
 (:translate-char-limit . "caratteri rimanenti")

 ;; Flashcard review
 (:review-title . "Ripassa")
 (:review-start . "Inizia ripasso")
 (:review-no-cards . "Nessuna carta da ripassare")
 (:review-show-answer . "Mostra risposta")
 (:review-again . "Ancora")
 (:review-hard . "Difficile")
 (:review-good . "Bene")
 (:review-easy . "Facile")
 (:review-progress . "Carta ~a di ~a")
 (:review-complete . "Ripasso completato!")
 (:review-complete-msg . "Hai ripassato ~a carte")
 (:review-streak . "giorni di serie")

 ;; Dashboard
 (:dashboard-title . "Dashboard")
 (:dashboard-cards-due . "Carte per oggi")
 (:dashboard-total-cards . "Carte totali")
 (:dashboard-subscription . "Abbonamento")
 (:dashboard-subscribed-until . "Abbonato fino al")
 (:dashboard-not-subscribed . "Non abbonato")
 (:dashboard-recent . "Traduzioni recenti")
 (:dashboard-no-translations . "Nessuna traduzione ancora")
 (:dashboard-streak . "Serie attuale")
 (:dashboard-days . "giorni")

 ;; Subscription
 (:subscribe-title . "Scegli il tuo piano")
 (:subscribe-monthly . "Mensile")
 (:subscribe-annual . "Annuale")
 (:subscribe-monthly-price . "10€/mese")
 (:subscribe-annual-price . "100€/anno")
 (:subscribe-monthly-desc . "600 traduzioni al mese")
 (:subscribe-annual-desc . "Risparmia 20€ - due mesi gratis!")
 (:subscribe-cta . "Abbonati")
 (:subscribe-success . "Abbonamento riuscito!")
 (:subscribe-manage . "Gestisci abbonamento")

 ;; Errors and status
 (:error-generic . "Qualcosa è andato storto")
 (:error-not-found . "Pagina non trovata")
 (:error-unauthorized . "Accedi per continuare")
 (:error-subscription-required . "Abbonamento richiesto")
 (:loading . "Caricamento...")
 (:empty-state . "Niente qui ancora")

 ;; Footer
 (:footer-privacy . "Privacy")
 (:footer-terms . "Termini di servizio")
 (:footer-contact . "Contatti")
)
